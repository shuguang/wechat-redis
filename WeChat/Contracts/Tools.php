...
class Tools
{
    ...
    public static function setCache($name, $value = '', $expired = 3600)
    {
        ...
        $file = self::_getCacheName(self::$key_prefix . $name);
        $data = ['name' => $name, 'value' => $value, 'expired' => time() + intval($expired)];
        $serialize = serialize($data);
        Redis::getInstance()->set(self::$key_prefix . $name, $serialize);
    ...
    public static function getCache($name)
    {
        ...
        //$file = self::_getCacheName($name);
        $data = Redis::getInstance()->get(self::$key_prefix . $name);
        if ($data) {
    ...
    public static function delCache($name)
    {
        ...
        Redis::getInstance()->del(self::$key_prefix . $name);
        return true;
        /*$file = self::_getCacheName($name);
